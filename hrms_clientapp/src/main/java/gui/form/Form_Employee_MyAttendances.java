...
public class Form_Employee_MyAttendances extends JPanel {
	private List<Attendance> list = null;

	public Form_Employee_MyAttendances() {
		initComponents();
	...
	private void initTableData() {
		...
		try {
			list = Application.attendanceDao.getListAttendanceByEmployee(Application.idUserLogin);
		...
		if (list != null) {
			for (Attendance attendance : list) {
				if (attendance.getTimeOut() != null) {
					...
					table1.addRow(new Object[] { dateTimeFormatter.format(attendance.getAttendancedDate()),
...
//        card4 = new gui.component.Card();
		...
		}, new String[] { "Date", "Employee", "Time In", "Time Out", "Total Hours", "Status (In/Out)" }) {
